...
$img_hozing_grid_3clumns  = wp_get_attachment_image_url( get_post_thumbnail_id(), 'hozing_room_grid_m' );
$img  = wp_get_attachment_image_url( get_post_thumbnail_id(), 'large' );

?>

<div class="rental_item item room_life_style col-lg-4 col-md-6">
    <div class="border-box">
        <div class="wrap_img">
            <div class="images_thumb">
               <img src="<?php echo esc_url($img_hozing_grid_3clumns);?>" />
            </div>
            <?php if( get_theme_mod( 'rl_type_show_button', 'true' ) == 'yes') : ?>
            <div class="discorver">
                <a href="<?php the_permalink(); ?> " class="discover-now">
                    <span class="text"><?php esc_html_e( 'Discover Now', 'hozing' ); ?></span>
                </a>
            </div>
            <?php endif; ?>
        </div> <!-- End wrap_img -->
        ...
        <div class="content ">
            <div class="d-inline-flex room-info">
                <h3 class="general_font"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h3>
            ...
            <?php 
                ...
                $ovacrs_specials_icon  = get_post_meta( get_the_id(), 'ovacrs_specials_icon', true );
                ...
                if( get_theme_mod( 'rl_type_show_special_info', 'true' ) == 'yes' ) : 
                    if( $ovacrs_specials_featured ){
                        foreach ($ovacrs_specials_featured as $key => $value) {
                            if( $value == 'yes' && trim( $ovacrs_specials_label[$key] ) != '' || trim( $ovacrs_specials_icon[$key] ) != '' ){
                            ?>
                                <li class="d-inline-flex">
                                    <?php if( trim( $ovacrs_specials_icon[$key] ) != '' ) : ?>
                                    <i class="<?php echo esc_attr( $ovacrs_specials_icon[$key] ); ?>"> </i>
                                    <?php endif; ?>
